...
def main():
    ...
    eigenvals = hessian_data['eigenvals']
    eigenvecs = hessian_data['eigenvecs']
    HPB.load_eigenthings(eigenvals[:-1], eigenvecs[:-1])
    ...
    torch.save(HPB.BRE.sigma_post_.detach().to('cpu'), 'sigma_post/hessian_patial_run_3.pth')
