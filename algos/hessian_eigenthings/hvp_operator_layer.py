...
class HVPOperator_layer(Operator):
    """
    ...
    def __init__(
        ...
        method='full',
        filterwise_normalization=False
    ):
        size = int(sum(p.numel() for p in model.parameters()))
        super(HVPOperator_layer, self).__init__(size)
        self.grad_vec = torch.zeros(size) # pylint: disable=no-member
        self.model = model
        if use_gpu:
        ...
        if end is not None:
            ...
            self.end=size
        assert method in ['full', 'block', 'cloumn', 'row', 'cross']
        self.method = method
        self.filterwise_normalization = filterwise_normalization
        if self.filterwise_normalization:
            self.filterwise_norm = self.filterwise_normalize()
    ...
    def apply(self, vec):
        ...
        """
        if self.filterwise_normalization:
            vec = vec.mul(self.filterwise_norm)
        if self.full_dataset:
            res = self._apply_full(vec)
        else:
            res = self._apply_batch(vec)
        if self.filterwise_normalization:
            res = res.mul_(self.filterwise_norm)
        return res

    def _apply_batch_origin(self, vec):
        ...
        hessian_vec_prod = torch.cat([g.contiguous().view(-1) for g in grad_grad]) # pylint: disable=no-member
        return hessian_vec_prod
    
    def truncate(self, vec):
        vec[:self.start] = torch.zeros_like(vec[:self.start]) # pylint: disable=no-member
        vec[self.end:] = torch.zeros_like(vec[self.end:]) # pylint: disable=no-member
        return vec

    def _apply_batch_block(self, vec):
        self.truncate(vec)
        hessian_vec_prod = self._apply_batch_origin(vec)
        self.truncate(hessian_vec_prod)
        return hessian_vec_prod

    def _apply_batch_column(self, vec):
        self.truncate(vec)
        hessian_vec_prod = self._apply_batch_origin(vec)
        return hessian_vec_prod
    
    def _apply_batch_row(self, vec):
        hessian_vec_prod = self._apply_batch_origin(vec)
        self.truncate(hessian_vec_prod)
        return hessian_vec_prod
    
    def _apply_batch_cross(self, vec):
        hessian_vec_prod = self._apply_batch_column(vec) + self._apply_batch_row(vec) - self._apply_batch_block(vec)
        return hessian_vec_prod

    def _apply_batch(self, vec):
        switcher = {
            'full': self._apply_batch_origin,
            'block': self._apply_batch_block,
            'column': self._apply_batch_column,
            'row': self._apply_batch_row,
            'cross': self._apply_batch_cross
        }
        return switcher.get(self.method)(vec)
    ...
    def prepare_grad(self):
        ...
        for input, target in zip(input_chunks, target_chunks):
            ...
            if grad_vec is not None:
                grad_vec += torch.cat([g.contiguous().view(-1) for g in grad_dict]) # pylint: disable=no-member
            else:
                grad_vec = torch.cat([g.contiguous().view(-1) for g in grad_dict]) # pylint: disable=no-member
        grad_vec /= num_chunks
    ...
    def filterwise_normalize(self, fc_rescale=True, bias_rescale=True):
        ...
        for opr in sd:
            if ('conv' in opr and '.weight' in opr and len(sd[opr].shape) == 4) or ('fc' in opr and '.weight' in opr and len(sd[opr].shape) == 2 and fc_rescale):
                for i in range(len(sd[opr])):
                    sd[opr][i] = torch.mul(torch.ones_like(sd[opr][i]), torch.norm(sd[opr][i])) # pylint: disable=maybe-no-member
            if bias_rescale:
                if '.bias' in opr:
                    assert sd[opr].dim() == 1, 'awkward dimension'
                    sd[opr] = torch.mul(torch.ones_like(sd[opr]), torch.norm(sd[opr])) # pylint: disable=maybe-no-member
        normalized_model = deepcopy(self.model)
        normalized_model.load_state_dict(sd)
        normalized_params, _ = get_params_grad(normalized_model)
        res = torch.cat([p.detach().flatten() for p in normalized_params])
        return res


def compute_hessian_eigenthings_layer(
    model,
    dataloader,
    loss,
    num_eigenthings=10,
    ...
    **kwargs:
    ...
    """
    hvp_operator_layer = HVPOperator_layer(
        model,
        dataloader,
        loss,
        use_gpu=use_gpu,
        ...
        method=method,
        filterwise_normalization=filterwise_normalization
    )
    eigenvals, eigenvecs = None, None
    if mode == "power_iter":
        eigenvals, eigenvecs = deflated_power_iteration(
            hvp_operator_layer, num_eigenthings, use_gpu=use_gpu, **kwargs
        )
    elif mode == "lanczos":
    ...
    eigenvecs = eigenvecs[:,start:end]
    eigenvecsnorm = np.linalg.norm(eigenvecs, axis=1).reshape((num_eigenthings, 1))
    eigenvecs = np.divide(eigenvecs, eigenvecsnorm)
